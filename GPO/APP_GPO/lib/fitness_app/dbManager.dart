...
class DbManager {
  ...
  void decrementaMed(int idMed) async {
    var db = await this.database;

    db.execute("UPDATE tableMedicine SET numPills = numPills-1 WHERE id='" +
        idMed.toString() +
        "'");

    //Controllo il numero di pillole rimaste: se sono 5 o 2 avviso che stanno per finire
    var result = await db.rawQuery(
        "SELECT numPills,name FROM tableMedicine WHERE id='" +
            idMed.toString() +
            "'");

    result.forEach((element) async {
      if (element['numPills'] <= 5) {
